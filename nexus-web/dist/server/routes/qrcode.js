...
function buildVlessUri(client, settings) {
...
function buildVmessUri(client, settings) {
    const host = settings?.server?.domain || settings?.server?.ip || 'server.example.com';
    const port = settings?.xray_port || 443;
    const uuid = client.uuid || client.username;
    const config = {
...
router.get('/:clientId', auth_1.requireAuth, async (req, res) => {
    ...
    switch (client.protocol) {
        case 'vless':
            uri = buildVlessUri(client, settings);
            break;
        case 'vmess':
            uri = buildVmessUri(client, settings);
            break;
        case 'trojan':
            uri = buildTrojanUri(client, settings);
            break;
        case 'ssh':
        case 'slowdns':
            uri = buildSshConfig(client, settings);
            break;
        default:
    ...
    try {
        const qrDataUrl = await qrcode_1.default.toDataURL(uri, {
